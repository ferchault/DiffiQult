''' Some minimal basis if you need to include one form an 
extra file  '''
# modfied UGBS: superset of basis functions for CBN, regular UGBS for H
modUGBS = {1: [('S', [(13739.0854166734          ,1.0000000)]),
('S', [(7016.36109438299          ,1.0000000)]),
('S', [(3583.15866840946          ,1.0000000)]),
('S', [(1829.86962476550          ,1.0000000)]),
('S', [( 934.489134729210         ,1.0000000)]),
('S', [( 477.230689612032         ,1.0000000)]),
('S', [( 243.715119463182         ,1.0000000)]),
('S', [( 124.461944187287         ,1.0000000)]),
('S', [(  63.5609952513411        ,1.0000000)]),
('S', [(  32.4597220758638        ,1.0000000)]),
('S', [(  16.5767315800501        ,1.0000000)]),
('S', [(   8.46550778330153       ,1.0000000)]),
('S', [(   4.32321786011102       ,1.0000000)]),
('S', [(   2.20780762884063       ,1.0000000)]),
('S', [(   1.12749685157938       ,1.0000000)]),
('S', [(   0.575797063890464      ,1.0000000)]),
('S', [(   0.294051604951678      ,1.0000000)]),
('S', [(   0.150168091845475      ,1.0000000)]),
('S', [(   7.668876968794858E-002 ,1.0000000)]),
('S', [(   3.916389509898707E-002 ,1.0000000)])],
7: [
('S', [( 395537.152566831            ,1.0000000)]),
('S', [( 201995.358823884            ,1.0000000)]),
('S', [( 103156.238855453            ,1.0000000)]),
('S', [(  52680.4659115022           ,1.0000000)]),
('S', [(  26903.1860742976           ,1.0000000)]),
('S', [(  13739.0854166734           ,1.0000000)]),
('S', [(   7016.36109438300          ,1.0000000)]),
('S', [(   3583.15866840946          ,1.0000000)]),
('S', [(   1829.86962476550          ,1.0000000)]),
('S', [(    934.489134729211         ,1.0000000)]),
('S', [(    477.230689612032         ,1.0000000)]),
('S', [(    243.715119463182         ,1.0000000)]),
('S', [(    124.461944187287         ,1.0000000)]),
('S', [(     63.5609952513412        ,1.0000000)]),
('S', [(     32.4597220758638        ,1.0000000)]),
('S', [(     16.5767315800501        ,1.0000000)]),
('S', [(      8.46550778330153       ,1.0000000)]),
('S', [(      4.32321786011102       ,1.0000000)]),
('S', [(      2.20780762884063       ,1.0000000)]),
('S', [(      1.12749685157938       ,1.0000000)]),
('S', [(      0.575797063890465      ,1.0000000)]),
('S', [(      0.294051604951678      ,1.0000000)]),
('S', [(      0.150168091845475      ,1.0000000)]),
('S', [(      7.668876968794860E-002 ,1.0000000)]),
('S', [(      3.916389509898707E-002 ,1.0000000)]),
('P', [(   1829.86962476550          ,1.0000000)]),
('P', [(    934.489134729211         ,1.0000000)]),
('P', [(    477.230689612032         ,1.0000000)]),
('P', [(    243.715119463182         ,1.0000000)]),
('P', [(    124.461944187287         ,1.0000000)]),
('P', [(     63.5609952513412        ,1.0000000)]),
('P', [(     32.4597220758638        ,1.0000000)]),
('P', [(     16.5767315800501        ,1.0000000)]),
('P', [(      8.46550778330153       ,1.0000000)]),
('P', [(      4.32321786011102       ,1.0000000)]),
('P', [(      2.20780762884063       ,1.0000000)]),
('P', [(      1.12749685157938       ,1.0000000)]),
('P', [(      0.575797063890465      ,1.0000000)]),
('P', [(      0.294051604951678      ,1.0000000)]),
('P', [(      0.150168091845475      ,1.0000000)]),
('P', [(      7.668876968794860E-002 ,1.0000000)]),
('P', [(      3.916389509898707E-002 ,1.0000000)])]}
modUGBS[5] = modUGBS[7]
modUGBS[6] = modUGBS[7]

def2TZVP = \
{1: [
('S', [(34.0613410, 0.60251978e-02)]),
('S', [(5.1235746          ,    0.45021094e-01)]),
('S', [(1.1646626         ,     0.20189726)]),
('S', [(0.32723041       ,      1.0000000)]),
('S', [(0.10307241      ,       1.0000000)]),
('S', [(0.8000000      ,        1.0000000)]),]}

basis_set_2G_STO_uncontracted = \
{1: [('S',
      [(1.309756377 ,0.430128498)]),     
     ('S',
      [(0.233135974 ,0.678913531)])], 
3: [('S', 
      [(6.1638450, 0.430128498)]), 
    ('S', 
      [( 1.0971610, 0.678913531)]),
    ('S', 
      [( 0.2459160, 0.678913531)]), 
    ('S', 
      [( 0.0623710, 0.678913531)]),
    ('P', 
      [( 0.259160, 0.678913531)]), 
    ('P', 
      [( 0.0623810, 0.678913531)])],
6: [('S',
       [(27.38503303,0.430128498)]),
     ('S',
       [(4.874522052,0.678913531)]),
     ('S',
       [(1.136748189,0.51154070)]),
     ('S',
       [(0.288309360,0.612819896)]),
     ('P',
       [(1.136748189,0.51154070)]),
     ('P',
       [(0.288309360,0.612819896)])],
7: [('S',
     [(37.8964750 ,0.4301280)]),    
     ('S',
     [( 6.7455540 ,0.6789140)]),    
     ('S',
     [( 1.4610890 ,0.0494720)]),    
     ('S',
     [( 0.3705710 ,0.9637820)]),    
     ('P',
     [(1.4610890  ,0.5115410)]),    
     ('P',
     [(0.3705710  ,0.6128200)])], 
8: [('S',
      [(49.9809710, 0.4301280)]),
     ('S',
       [(8.8965880, 0.6789140)]),
     ('S',
       [(1.9452370,0.0494720)]),
     ('S',
       [(0.4933630 ,0.9637820)]),
     ('P',
       [(1.9452370,0.0494720)]),
     ('P',
       [(0.4933630 ,0.9637820)])],
9: [('S',
      [(63.7352020, 0.4301280)]),
     ('S',
       [(11.3448340, 0.6789140)]),
     ('S',
       [(2.4985480, 0.6789140)]),
     ('S',
       [(0.6336980, 0.6789140)]),
     ('P',
       [(2.4985480, 0.6789140)]),
     ('P',
       [(0.6336980, 0.6789140)])]
}

basis_set_3G_STO_uncontracted = \
{1: [('S',
      [(3.42525091,0.15432897)]),    
     ('S',
      [(0.62391373,0.53532814)]),       
     ('S',
      [(0.16885540,0.44463454)])],
3: [('S', 
      [(16.1195750, 0.15432897)]),    
     ('S',
      [( 2.9362007, 0.53532814)]),    
     ('S',
      [( 0.7946505, 0.44463454)]),
     ('S',
      [(0.6362897,-0.09996723)]),   
     ('S',
      [(0.1478601, 0.39951283)]),    
     ('S',
      [(0.0480887, 0.70011547)]),  
     ('P',
      [(0.6362897, 0.15591627)]),   
     ('P',
      [(0.1478601, 0.60768372)]),    
     ('P',
      [(0.0480887, 0.39195739)])],
6: [('S',
      [(71.6168370,0.15432897)]),   
   ('S',
      [(13.0450960,0.53532814)]),   
   ('S',
      [(3.5305122,0.44463454)]),
   ('S',
      [(2.9412494,-0.09996723)]),   
   ('S',
      [(0.6834831, 0.39951283)]),   
   ('S',
      [(0.2222899, 0.70011547)]),   
   ('P',
      [(2.9412494, 0.15591627)]),   
   ('P',
      [(0.6834831, 0.60768372)]),   
   ('P',
      [(0.2222899, 0.39195739)])],   
7: [('S',
      [(  99.1061690, 0.15432897)]),   
   ('S',
      [(  18.0523120, 0.53532814)]),   
   ('S',
      [(   4.8856602, 0.44463454)]),   
   ('S',
      [(   3.7804559,-0.09996723)]),   
   ('S',
      [(   0.8784966, 0.39951283)]),   
   ('S',
      [(   0.2857144, 0.70011547)]),   
   ('P',
      [(   3.7804559, 0.15591627)]),   
   ('P',
      [(   0.8784966, 0.60768372)]),   
   ('P',
      [(   0.2857144, 0.39195739)])],
8: [('S',
      [( 130.7093200, 0.15432897)]),   
   ('S',
      [(  23.8088610, 0.53532814)]),   
   ('S',
      [(   6.4436083, 0.44463454)]),   
   ('S',
      [(   5.0331513,-0.09996723)]),   
   ('S',
      [(   1.1695961, 0.39951283)]),   
   ('S',
      [(   0.3803890, 0.70011547)]),   
   ('P',
      [(   5.0331513, 0.15591627)]),   
   ('P',
      [(   1.1695961, 0.60768372)]),   
   ('P',
      [(   0.3803890, 0.39195739)])],   
9: [('S',
      [( 166.6791300, 0.15432897)]),   
   ('S',
      [(  30.3608120, 0.53532814)]),   
   ('S',
      [(   8.2168207, 0.44463454)]),   
   ('S',
      [(   6.4648032,-0.09996723)]),   
   ('S',
      [(   1.5022812, 0.39951283)]),   
   ('S',
      [(   0.4885885, 0.70011547)]),   
   ('P',
      [(   6.4648032, 0.15591627)]),   
   ('P',
      [(   1.5022812, 0.60768372)]),   
   ('P',
      [(   0.4885885, 0.39195739)])]
}   


basis_set_6G_STO_uncontracted = \
{1: [('S',
 [(  35.52322122,           0.00916359628)]),
   ('S',
 [(  6.513143725,           0.04936149294)]),
   ('S',
 [(  1.822142904,           0.16853830490)]),
   ('S',
 [(  0.625955266,           0.37056279970)]),
   ('S',
 [(  0.243076747,           0.41649152980)]),
   ('S',
 [(  0.100112428,           0.13033408410)])],
3: [('S',
 [( 167.17584620  ,          0.00916359628)]),
   ('S',
 [(  30.651508400 ,          0.04936149294)]),
   ('S',
 [(   8.5751874770,          0.16853830490)]),
   ('S',
 [(   2.9458083370,          0.37056279970)]),
   ('S',
 [(   1.1439435810,          0.41649152980)]),
   ('S',
 [(   0.4711391391,          0.13033408410)]),
   ('S',
 [(   6.5975639810,         -0.01325278809)]),
   ('S',
 [(   1.3058300920,         -0.04699171014)]),
   ('S',
 [(   0.4058510193,         -0.03378537151)]),
   ('S',
 [(   0.1561455158,          0.25024178610)]),
   ('S',
 [(   0.0678141039,          0.59511725260)]),
   ('S',
 [(   0.0310841655,          0.24070617630)]),
   ('P',
 [(   6.5975639810,          0.0037596966 )]),
   ('P',
 [(   1.3058300920,          0.0376793698 )]),
   ('P',
 [(   0.4058510193,          0.1738967435 )]),
   ('P',
 [(   0.1561455158,          0.4180364347 )]),
   ('P',
 [(   0.0678141039,          0.4258595477 )]),
   ('P',
 [(   0.0310841655,          0.1017082955 )])],
6: [('S',
 [( 742.7370491   ,          0.00916359628)]),
   ('S',
 [( 136.1800249   ,          0.04936149294)]),
   ('S',
 [(  38.09826352  ,          0.16853830490)]),
   ('S',
 [(  13.08778177  ,          0.37056279970)]),
   ('S',
 [(   5.082368648 ,          0.41649152980)]),
   ('S',
 [(   2.093200076 ,          0.13033408410)]),
   ('S',
 [(  30.49723950  ,         -0.01325278809)]),
   ('S',
 [(   6.036199601 ,         -0.04699171014)]),
   ('S',
 [(   1.876046337 ,         -0.03378537151)]),
   ('S',
 [(   0.7217826470,          0.25024178610)]),
   ('S',
 [(   0.3134706954,          0.59511725260)]),
   ('S',
 [(   0.1436865550,          0.24070617630)]),
   ('P',
 [(  30.49723950  ,          0.0037596966 )]),
   ('P',
 [(   6.036199601 ,          0.0376793698 )]),
   ('P',
 [(   1.876046337 ,          0.1738967435 )]),
   ('P',
 [(   0.7217826470,          0.4180364347 )]),
   ('P',
 [(   0.3134706954,          0.4258595477 )]),
   ('P',
 [(   0.1436865550,          0.1017082955 )])],
7: [('S',
 [(1027.8284580   ,          0.00916359628)]),
   ('S',
 [( 188.4512226   ,          0.04936149294)]),
   ('S',
 [(  52.72186097  ,          0.16853830490)]),
   ('S',
 [(  18.11138217  ,          0.37056279970)]),
   ('S',
 [(   7.033179691 ,          0.41649152980)]),
   ('S',
 [(   2.896651794 ,          0.13033408410)]),
   ('S',
 [(  39.19880787  ,         -0.01325278809)]),
   ('S',
 [(   7.758467071 ,         -0.04699171014)]),
   ('S',
 [(   2.411325783 ,         -0.03378537151)]),
   ('S',
 [(   0.9277239437,          0.25024178610)]),
   ('S',
 [(   0.4029111410,          0.59511725260)]),
   ('S',
 [(   0.1846836552,          0.24070617630)]),
   ('P',
 [(  39.19880787  ,          0.0037596966 )]),
   ('P',
 [(   7.758467071 ,          0.0376793698 )]),
   ('P',
 [(   2.411325783 ,          0.1738967435 )]),
   ('P',
 [(   0.9277239437,          0.4180364347 )]),
   ('P',
 [(   0.4029111410,          0.4258595477 )]),
   ('P',
 [(   0.1846836552,          0.1017082955 )])],
8: [('S',
 [(1355.5842340   ,          0.00916359628)]),
   ('S',
 [( 248.5448855   ,          0.04936149294)]),
   ('S',
 [(  69.53390229  ,          0.16853830490)]),
   ('S',
 [(  23.88677211  ,          0.37056279970)]),
   ('S',
 [(   9.275932609 ,          0.41649152980)]),
   ('S',
 [(   3.820341298 ,          0.13033408410)]),
   ('S',
 [(  52.18776196  ,         -0.01325278809)]),
   ('S',
 [(  10.32932006  ,         -0.04699171014)]),
   ('S',
 [(   3.210344977 ,         -0.03378537151)]),
   ('S',
 [(   1.235135428 ,          0.25024178610)]),
   ('S',
 [(   0.536420158 ,          0.59511725260)]),
   ('S',
 [(   0.245880606 ,          0.24070617630)]),
   ('P',
 [(  52.18776196  ,          0.0037596966 )]),
   ('P',
 [(  10.32932006  ,          0.0376793698 )]),
   ('P',
 [(   3.210344977 ,          0.1738967435 )]),
   ('P',
 [(   1.235135428 ,          0.4180364347 )]),
   ('P',
 [(   0.536420158 ,          0.4258595477 )]),
   ('P',
 [(   0.245880606 ,          0.1017082955 )])],
9: [('S',
 [(1728.6265740   ,          0.00916359628)]),
   ('S',
 [( 316.9417900   ,          0.04936149294)]),
   ('S',
 [(  88.66889139  ,          0.16853830490)]),
   ('S',
 [(  30.46015731  ,          0.37056279970)]),
   ('S',
 [(  11.82857044  ,          0.41649152980)]),
   ('S',
 [(   4.871658522 ,          0.13033408410)]),
   ('S',
 [(  67.03228091  ,         -0.01325278809)]),
   ('S',
 [(  13.26743777  ,         -0.04699171014)]),
   ('S',
 [(   4.123509771 ,         -0.03378537151)]),
   ('S',
 [(   1.586462839 ,          0.25024178610)]),
   ('S',
 [(   0.689001892 ,          0.59511725260)]),
   ('S',
 [(   0.315819978 ,          0.24070617630)]),
   ('P',
 [(  67.03228091  ,          0.0037596966 )]),
   ('P',
 [(  13.26743777  ,          0.0376793698 )]),
   ('P',
 [(   4.123509771 ,          0.1738967435 )]),
   ('P',
 [(   1.586462839 ,          0.4180364347 )]),
   ('P',
 [(   0.689001892 ,          0.4258595477 )]),
   ('P',
 [(   0.315819978 ,          0.1017082955 )])]
}

basis_set_3G_STO = \
{1: [('S',
      [(3.4252509099999999, 0.15432897000000001),
       (0.62391373000000006, 0.53532813999999995),
       (0.16885539999999999, 0.44463454000000002)])],
 2: [('S',
      [(6.3624213899999997, 0.15432897000000001),
       (1.1589229999999999, 0.53532813999999995),
       (0.31364978999999998, 0.44463454000000002)])],
 3: [('S',
      [(16.119575000000001, 0.15432897000000001),
       (2.9362007000000001, 0.53532813999999995),
       (0.79465050000000004, 0.44463454000000002)]),
     ('S',
      [(0.63628969999999996, -0.099967230000000004),
       (0.14786009999999999, 0.39951282999999999),
       (0.048088699999999998, 0.70011546999999996)]),
     ('P',
      [(0.63628969999999996, 0.15591627),
       (0.14786009999999999, 0.60768372000000004),
       (0.048088699999999998, 0.39195739000000002)])],
 4: [('S',
      [(30.167871000000002, 0.15432897000000001),
       (5.4951153000000001, 0.53532813999999995),
       (1.4871927, 0.44463454000000002)]),
     ('S',
      [(1.3148331, -0.099967230000000004),
       (0.3055389, 0.39951282999999999),
       (0.099370700000000006, 0.70011546999999996)]),
     ('P',
      [(1.3148331, 0.15591627),
       (0.3055389, 0.60768372000000004),
       (0.099370700000000006, 0.39195739000000002)])],
 5: [('S',
      [(48.791113000000003, 0.15432897000000001),
       (8.8873622000000001, 0.53532813999999995),
       (2.4052669999999998, 0.44463454000000002)]),
     ('S',
      [(2.2369561, -0.099967230000000004),
       (0.51982050000000002, 0.39951282999999999),
       (0.16906180000000001, 0.70011546999999996)]),
     ('P',
      [(2.2369561, 0.15591627),
       (0.51982050000000002, 0.60768372000000004),
       (0.16906180000000001, 0.39195739000000002)])],
 6: [('S',
      [(71.616837000000004, 0.15432897000000001),
       (13.045095999999999, 0.53532813999999995),
       (3.5305122, 0.44463454000000002)]),
     ('S',
      [(2.9412493999999998, -0.099967230000000004),
       (0.68348310000000001, 0.39951282999999999),
       (0.22228990000000001, 0.70011546999999996)]),
     ('P',
      [(2.9412493999999998, 0.15591627),
       (0.68348310000000001, 0.60768372000000004),
       (0.22228990000000001, 0.39195739000000002)])],
 7: [('S',
      [(99.106168999999994, 0.15432897000000001),
       (18.052312000000001, 0.53532813999999995),
       (4.8856602000000002, 0.44463454000000002)]),
     ('S',
      [(3.7804559000000002, -0.099967230000000004),
       (0.87849659999999996, 0.39951282999999999),
       (0.28571439999999998, 0.70011546999999996)]),
     ('P',
      [(3.7804559000000002, 0.15591627),
       (0.87849659999999996, 0.60768372000000004),
       (0.28571439999999998, 0.39195739000000002)])],
 8: [('S',
      [(130.70931999999999, 0.15432897000000001),
       (23.808861, 0.53532813999999995),
       (6.4436083000000002, 0.44463454000000002)]),
     ('S',
      [(5.0331513000000001, -0.099967230000000004),
       (1.1695960999999999, 0.39951282999999999),
       (0.38038899999999998, 0.70011546999999996)]),
     ('P',
      [(5.0331513000000001, 0.15591627),
       (1.1695960999999999, 0.60768372000000004),
       (0.38038899999999998, 0.39195739000000002)])],
 9: [('S',
      [(166.67912999999999, 0.15432897000000001),
       (30.360811999999999, 0.53532813999999995),
       (8.2168206999999995, 0.44463454000000002)]),
     ('S',
      [(6.4648032000000004, -0.099967230000000004),
       (1.5022812000000001, 0.39951282999999999),
       (0.48858849999999998, 0.70011546999999996)]),
     ('P',
      [(6.4648032000000004, 0.15591627),
       (1.5022812000000001, 0.60768372000000004),
       (0.48858849999999998, 0.39195739000000002)])],
 10: [('S',
       [(207.01561000000001, 0.15432897000000001),
        (37.708151000000001, 0.53532813999999995),
        (10.205297, 0.44463454000000002)]),
      ('S',
       [(8.2463151000000003, -0.099967230000000004),
        (1.9162661999999999, 0.39951282999999999),
        (0.62322929999999999, 0.70011546999999996)]),
      ('P',
       [(8.2463151000000003, 0.15591627),
        (1.9162661999999999, 0.60768372000000004),
        (0.62322929999999999, 0.39195739000000002)])]}

